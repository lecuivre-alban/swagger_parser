...
final class DartFreezedDtoTemplate {
  const DartFreezedDtoTemplate(this.enums);

  final List<UniversalEnumClass> enums;
...
''');
  ...
  String? _validationString(UniversalType type) {
    ...
    if (type.minLength != null) {
      sb.write(
          '  static const int ${type.name}MinLength = ${type.minLength};\n');
    }

    if (type.maxLength != null) {
      sb.write(
          '  static const int ${type.name}MaxLength = ${type.maxLength};\n');
    }
  ...
  String _defaultValue(UniversalType t) {
    final enumType = enums.firstWhereOrNull((e) => e.name == t.type);

    if (enumType != null) {
      final defaultValueMatch = RegExp(
        r'(?<=\[).*(?=\])',
      ).stringMatch(t.defaultValue!);

      if (defaultValueMatch case final match?) {
        if (match.isEmpty) {
          return '[]';
        }

        final values = match.split(',').map((e) => e.trim()).toList();
        return '[${protectEnumItemsNames(values).map((e) => '${enumType.name}.${e.name.toCamel}').join(', ')}]';
      } else {
        return '${enumType.name}.${protectDefaultEnum(t.defaultValue)?.toCamel}';
      }
    } else {
      return protectDefaultValue(t.defaultValue, type: t.type) ?? '';
    }
  }
}
