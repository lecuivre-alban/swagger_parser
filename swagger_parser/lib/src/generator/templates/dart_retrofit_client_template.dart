...
final class DartRetrofitTemplate {
  const DartRetrofitTemplate(this.enums);
...
''',
  ...
  String _fileImport(UniversalRestClient restClient) => restClient.requests.any(
        (r) => r.parameters.any(
          (e) =>
              e.type.toSuitableType(ProgrammingLanguage.dart).contains('File'),
        ),
      )
          ? "import 'dart:io';\n\n"
          : '';
  ...
  String _defaultValue(UniversalType t) {
    ...
    if (t.defaultValue case final defaultValue?) {
      ...
      if (RegExp(r'(?<=\[).*(?=\])').stringMatch(defaultValue)?.trim()
          case final match?) {
        if (match.isEmpty) {
          value += '[]';
        } else {
          value += '[';
          if (enums.firstWhereOrNull((e) => e.name == t.type)
              case final enumType?) {
            final values = match.split(',').map((e) => e.trim()).toList();
            value += protectEnumItemsNames(values)
                .map((e) => '${enumType.name}.${e.name.toCamel}')
                .join(', ');
          } else {
